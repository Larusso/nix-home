...

{
  ...
  programs.zsh = {
    ...
    localVariables = {
      POWERLEVEL9K_LEFT_PROMPT_ELEMENTS =
        [ "os_icon" "dir" "vcs" "newline" "status" ];
      POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS =
        [ "direnv" "asdf" "pyenv" "rbenv" "nodenv" "nix_shell" ];
  ...
  programs.git = {
    ...
    aliases = {
      ...
      "next" = ''
        !git co $(git log master --reverse --ancestry-path HEAD..master --pretty=tformat:"%H" | head -1)'';
      "prev" =
        "!git co $(git rev-list --first-parent --max-count=2 HEAD | tail -1)";
